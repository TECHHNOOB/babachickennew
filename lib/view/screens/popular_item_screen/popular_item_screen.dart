...
class _PopularItemScreenState extends State<PopularItemScreen> {
  ...
  Widget build(BuildContext context) {
      ...
      if (_scrollController.position.pixels ==
        ...
        if (productProvider.popularOffset < pageSize) {
          ...
          productProvider.getPopularProductList(
              false, productProvider.popularOffset.toString(),
              type: _type);
        }
      }
    });
    return Scaffold(
      appBar: CustomAppBar(
          context: context, title: getTranslated('popular_item', context)),
      body:
          ...
          children: [
            ...
            Expanded(
              child: productProvider.popularProductList != null
                  ? productProvider.popularProductList!.isNotEmpty
                      ? RefreshIndicator(
                          onRefresh: () async {
                            _type = 'all';
